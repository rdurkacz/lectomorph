...
with DICTIONARY_PACKAGE; use DICTIONARY_PACKAGE;
  function DICTIONARY_FORM1(DE : DICTIONARY_ENTRY) return STRING is
      ...
      ordinal: array (WHICH_TYPE range 1..5) of STRING(1..3) :=
        ("1st", "2nd", "3rd", "4th", "5th");
...
--              OX(2) := ADD(DE.STEMS(2), "iri");   --  In practice there is no problem
          ...
          --not DEP  
          elsif DE.PART.V.KIND = PERFDEF  then
  ...
  --          OX(4) := ADD(NULL_OX, "PERFDEF");
          else                            --  Not DEP/PERFDEF/IMPERSzzz  
...
--              OX(2) := ADD(DE.STEMS(2), "ire");
              ...
              OX(1) := ADD(DE.STEMS(1), "");              --revisiting OX1
              OX(2) := ADD(NULL_OX, "undeclined");
...
--  For DICTPAGE
        if DE.PART.N.DECL.WHICH in 1..5 and
           DE.PART.N.DECL.VAR  in 1..5 then
             ADD_TO(" (" & ordinal(DE.PART.N.DECL.WHICH) & ")");
         end if;         
 ...
 --  For DICTPAGE
        if DE.PART.V.CON.WHICH in 1..3 then
           if DE.PART.V.CON.VAR = 1 then
             ADD_TO(" (" & ordinal(DE.PART.V.CON.WHICH) & ")");
